...
function main() {
    // console.debug("main");
    const clutCanvas = document.getElementById('clutCanvas');
    const srcCanvas = document.getElementById('srcCanvas');
    const grayCanvas = document.getElementById('grayCanvas');
    const dstCanvas = document.getElementById('dstCanvas');
    let srcImage = new Image(srcCanvas.width, srcCanvas.height);
    let clutType = document.getElementById('clutTypeSelect').value;
...
function makeCLUTfromCRGB(points) {
    const table = [];
    for (let j = 0; j < 256; j++) {
	let prevRatio = 0; let nextRatio = 0;
	let prevIndex = 0; let nextIndex = 0;
	for (let k = prevIndex, n = points.length; k < n; k++) {
	...
	const prevRGB = points[prevIndex];
	if (prevRatio < nextRatio) {
	    const nextRGB = points[nextIndex];
	    const r = (j - prevRatio) / (nextRatio - prevRatio);
	    const lPrevRGB = sRGB2linearRGB(prevRGB.slice(1));
	    const lNextRGB = sRGB2linearRGB(nextRGB.slice(1));
	    const lrgb = [
...
function makeCLUT() {
    for (const name in CLUTtemplate) {
	const points = CLUTtemplate[name];
...
function drawPseudoColor(grayCanvas, dstCanvas, clutType) {
    ...
    const width = grayCanvas.width; const height = grayCanvas.height;
    ...
    for (let y = 0; y < height; y++) {
	    ...
	    let [r, g, b, a] = getRGBA(grayImageData, x, y);
