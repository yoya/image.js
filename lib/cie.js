...
function drawSpectrumGraphAxis(canvas, params) {
    ...
    var y0 = height * (1 - (0 -yMin) / (yMax-yMin));
    ...
    for (var v = -3 ;  v <= 5 ; v += 1) {
        ctx.beginPath();
        var y = height * (1 - (v -yMin) / (yMax-yMin));
	if (v == 0) {
            console.log(v);
	    ctx.strokeStyle= "rgb(192,192,192)";
	} else {
...
function drawSpectrumGraphCMF(canvas, params) {
    ...
    var graphLines = [["#F88", lxArr], ["#8F8", lyArr], ["#48F", lzArr]];
    // ctx.globalCompositeOperation = "lighter";
