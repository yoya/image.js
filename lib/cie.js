...
var loadCIEXYZdata = function(callback) {
	...
	xhr.onreadystatechange = function() {
		...
		if (cie === "31") { // cieSelect as default
		    callback('cie31Arr', arr, true);
		} else if (cie === "64") {
		    callback('cie64Arr', arr, false);
		} else { // "jv"
		    callback('cieJVArr', arr, false);
		}
	    }
	};
	xhr.cie = cie;
...
function drawGraph(canvas, params, sync) {
    canvas.width  = canvas.width ; // clear
    drawGraphBase(canvas, params, sync);
}
...
function drawDiagramBase(dstCanvas, params, sync) {
...
function drawDiagramPoints(canvas, params, sync) {
	...
	for (var i in drawPoints) {
	    ...
	    var color = point.color, xy = point.xy;
	    ctx.beginPath();
	    ctx.strokeStyle = color;
	    ctx.fillStyle = color;
	    ctx.lineWidth = 0.5;
	    var [gx, gy] = graphTrans(xy, width, height);
	    ctx.arc(gx, gy, 5, 0, 2*Math.PI, false);
	    ctx.stroke();
	}
    }
}

function drawGraphBase(canvas, params, sync) {
    var cieArr   = params['cieArr'];
