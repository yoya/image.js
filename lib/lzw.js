...
class IO_GIF_LZW {
    ...
    DGifBufferedInput() {
        if (this._data.length <= this._dataOffset) {
            throw "D_GIF_ERR_READ_FAILED";
        }
        return this._data[this._data_offset++];
    }
    ...
    dumpLZWCode_Giflib(LZWcode, codeBits, Indices) {
        // DGifBufferedInput initialize
        this._data = LZWcode;
        this._data_offset = 0;
        this.DGifSetupDecompress(codeBits);
        let StackPtr  = this.StackPtr;
        let Prefix    = this.Prefix;
        let Suffix    = this.Suffix;
        let Stack     = this.Stack;
        let EOFCode   = this.EOFCode;
        let ClearCode = this.ClearCode;
