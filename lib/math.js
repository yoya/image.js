...
function homographyCoeffByMarkersToSquare(markers) {
    ...
    let de_denom = dx1101 * dy1000 - dx1000 * dy1110;
    let gh01 = (dx1000*dy1101 - dx1101*dy1000) / (dx1101*dy0100 - dx0100*dy1101);
    let gh10 = (dx0100*dy1110 - dx1101*dy1000) / (dx1110*dy1000 - dx1000*dy1110);
    let a =  (dy0100/delta) * (abcd_num / ab_denom);
    let b = -(dx0100/delta) * (abcd_num / ab_denom);
    let d =  (dy1000/delta) * (abcd_num / de_denom);
    let e = -(dx1000/delta) * (abcd_num / de_denom);
