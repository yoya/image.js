...
class Binary {
    ...
    readUint8(arr, offset) {
	return arr[offset];
    }
    readSint8(arr, offset) {
	var v = this.readUint8(arr, offset);
	return (v < 0x80)? v :(v - 0x100);
    }
    ...
    readText(arr, offset, nBytes) {
	var arr = arr.subarray(offset, offset, offset + nBytes);
	if (nBytes < 100*1024) { // macOS+Chrome: 122KB OK, 123KB NG
	    return String.fromCharCode.apply(null, arr);
	} else {
	    var s = [];
	    for (var i = 0  ; i < nBytes ; i++) {
