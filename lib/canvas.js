...
function getRGBA(imageData, x, y, outfill) {
	...
	default:
	case "white": // white fill
	    return [255, 255, 255, 255];
	    break;
	case "transparent": // transparent fill
	    return [0, 0, 0, 0];
	    break;
	case "edge": // edge extend fill
	    x = clamp(x, 0, width - 1);
	    y = clamp(y, 0, height - 1);
	    break;
	case "tile": // tiled repeat fill
	    x %= width;
	    y %= height;
	    break;
	case "mirror": // mirror tiled repeat fill
	    if (x < 0) {
		x = -x;
	    }
	    if (y < 0) {
