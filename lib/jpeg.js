...
class IO_JPEG {
	...
	while (bo < arrLen) {
		...
		case 0xE0: // APP0
			...
			var version = "" + arr[o] + "." + arr[o+1];
			infos.push({offset:o, version:version});
			o += 2;
		    } else {
			var extensionCode = arr[o];
			var extentionData = arr.slice(o+1, bo+2+len - (o+1));
			var extensionDataStr = Utils.ToHexArray(extentionData).join(",")
	...
	for (let chunk of this.chunkList) {
	    if (chunk.name === "APP0") {
                ...
                var jfifSig = String.fromCharCode.apply(null, bytes.subarray(4, 4+5));
                if (jfifSig === "JFIF\0") {
                    return bytes.subarray(4 + 5);
                }
            }
        }
    }
	...
	for (let chunk of this.chunkList) {
	    if (chunk.name === "APP1") {
                const bytes = chunk.bytes;
                var jfifSig = String.fromCharCode.apply(null, bytes.subarray(4, 4+6));
                if (jfifSig === "Exif\0\0") {
                    return bytes.subarray(4 + 6);
                } else {
                    console.warn("unknown APP1 chunk:", { jfifSig });
                }
            }
        }
    }
    getICC() {
	var iccIdx = 0, iccCount = 0;
	var iccBlockList = [];
	var iccProfileSize = 0;
	for (var idx in this.chunkList) {
	    var chunk = this.chunkList[idx];
	    if (chunk.name === "APP2") {
		var bytes = chunk.bytes;
		var iccpSig = String.fromCharCode.apply(null, bytes.subarray(4, 16));
		if (iccpSig === "ICC_PROFILE\0") {
			...
			iccProfileSize += blockBytes.length;
                    ...
                    console.warn("unknown APP2 chunk:", { iccpSig });
                }
	    }
	}
	if (iccCount < 1) {
	...
	for (let chunk of this.chunkList) {
	    ...
	    if (chunk.name === "APP14") {
                const bytes = chunk.bytes;
                var adobeSig = String.fromCharCode.apply(null, bytes.subarray(4, 4 + 5));
		if (adobeSig === "Adobe") {
                    return bytes.subarray(4 + 5);
                } else {
                    console.warn("unknown APP14 chunk:", { adobeSig });
                }
                return bytes;
            }
        }
