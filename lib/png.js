...
class IO_PNG {
	...
	while (bo < arrLen) {
		...
		case "IHDR":
		    ...
		    break;
                case "iDOT":
                    var data1 = this.binary.readUint32(arr, o);
                    var data2 = this.binary.readUint32(arr, o+4);
                    var data3 = this.binary.readUint32(arr, o+8);
                    var data4 = this.binary.readUint32(arr, o+12);
		    infos.push({offset:o, data1:data1});
                    infos.push({offset:o+4, data2:data2});
                    infos.push({offset:o+8, data3:data3});
                    infos.push({offset:o+12, data4:data4});
                    break;
		default:
    ...
    getImageWidth() {
        const ihdrChunk = this.getIHDRchunk();
        return ihdrChunk.infos[2].width;
    }
    getImageHeight() {
        const ihdrChunk = this.getIHDRchunk();
