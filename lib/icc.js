...
class IO_ICC {
	...
	if (type !== 'desc') {
	    console.error("type is not desc:"+type);
	    return null;
	}
	...
	var type = binary.getText(4);
	if (type !== 'text') {
	    console.error("type is not text:"+type);
	    return {};
	}
	binary.getSubArray(4); // skip
	...
	var type = binary.getText(4);
	if (type !== 'mluc') {
	    console.error("type is not mluc:"+type);
	    return {};
	}
	binary.getSubArray(4); // skip
	...
	records.toString = function() {
		...
		var record = this[i];
		s.push(record['LangCode']+":"+record['CountryCode']+":"+record['String']);
	    }
	    return s.join(", ");
	}
	...
	var type = binary.getText(4);
	if (type !== 'XYZ ') {
	    console.error("type is not XYZ :"+type);
	    return {};
	}
	binary.getSubArray(4); // skip
	...
	var type = binary.getText(4);
	if (type !== 'curv') {
	    console.error("type is not curv:"+type);
	    return null;
	}
	binary.getSubArray(4); // skip
	...
	var type = binary.getText(4);
	if (type !== 'para') {
	    console.error("type is not para:"+type);
	    return null;
	}
	binary.getSubArray(4); // skip
	...
	var type = binary.getText(4);
	if (type !== 'sf32') {
	    console.error("type is not sf32:"+type);
	    return null;
	}
	binary.getSubArray(4); // skip
