...
function xyz2fisheye(xyz, width, height, srcProjX, srcProjY, srcProjR) {
    ...
    var xx = ((phi2_over_pi * Math.cos(theta)) + srcProjX) * width;
    var yy = ((phi2_over_pi * Math.sin(theta)) + srcProjY) * height;
    xx = (xx - width/2) *  srcProjR + width/2;
    yy = (yy - height/2) * srcProjR + height/2;
    return [xx, yy];
}
