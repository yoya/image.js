...
function setTableFunction(tableId, values, width) {
...
function bindTableFunction(tableId, callback, values, width) {
    ...
    setTableFunction(tableId, values, width);
    for (var i = 0 ; i < n ; i++) {
	var map = {}
	map[valueIdPrefix+i] = null;
	bindFunction(map,
		     function(target) {
			 var i = target.id.substr(valueIdPrefix.length);
