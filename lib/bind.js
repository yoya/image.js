...
function bindFunction(idMap, callback, params){
    ...
    let entries = new URL(document.location).searchParams.entries();
    for (const [id, value] of entries) {
	let elem = document.getElementById(id);
        let value2 = value;
	if (!elem) {
	    console.warn("element:"+id+" is not found");
	    return ;
	}
	...
	case "checkbox":
            ...
            elem.checked = value2;
            console.log(elem.checked, value, value2);
	    break;
        case "color":
	case "text":
            // nothing to convert value type
            elem.value = value2;
	    break;
	case "select-one":
	case "select-multiple":
	default:
	    console.error("Unknown URL parameter elem type:"+type+", id:"+id);
	    break;
	}
        params[id] = value2;
    }
